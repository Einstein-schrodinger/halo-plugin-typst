...
Typst 插件（更准确的说是Typst）可以用来渲染各式各样的排版内容，通过选择合适的 Template 可以轻松渲染精美的学术笔记、期刊论文、简历、海报等，此外，该插件也适合渲染片段代码来作为博客文章的插图，借助各种 Package 可以实现任何你想要的效果，比如借助 CeTZ 等 Package 可以轻松绘制各种图像，例如流程图、电路图、五线谱图、二维函数图、三维函数图等。
